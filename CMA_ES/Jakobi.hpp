...
void eigenValues_eigenVectors(const LMatrix& X, std::vector<double>& eigenValues, LMatrix& eigenVectors) {
    // wrapper for our stuff
    std::vector<double> eigenVectors_stdvec(X.n * X.m, 0);
    int trash1, trash2;
    jacobi_eigenvalue(X.n, &X.getLine()[0], 1000, &eigenVectors_stdvec[0], &eigenValues[0], trash1, trash2);

    eigenVectors.setLine(eigenVectors_stdvec);
}
